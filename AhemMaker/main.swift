...
let ascenderSquare: Path = [[Point(x: 0, y: 800), Point(x: 1000, y: 800), Point(x: 1000, y: 200), Point(x: 0, y: 200)]]
let descenderSquare: Path = [[Point(x: 0, y: 200), Point(x: 1000, y: 200), Point(x: 1000, y: -200), Point(x: 0, y: -200)]]
...
func baseTable() -> Data {
    ...
    append(result, value: UInt16(1)) // Major version
    append(result, value: UInt16(0)) // Minor version
    append(result, value: UInt16(8)) // Offset to horizontal data
    append(result, value: UInt16(0)) // Offset to vertical data (invalid => ignored)

    let axisTableLocation = result.length
    append(result, value: UInt16(4)) // Offset to BaseTagList table
    append(result, value: UInt16(0)) // Placeholder for offset to BaseScriptList table

    append(result, value: UInt16(1)) // Number of baseline identification tags
    let romanBaselineTag = FourCharacterTag(string: "romn")
    append(result, value: romanBaselineTag.a)
    append(result, value: romanBaselineTag.b)
    append(result, value: romanBaselineTag.c)
    append(result, value: romanBaselineTag.d)

    let baseScriptListLocation = result.length
    overwrite(result, location: axisTableLocation + 2, value: UInt16(result.length - axisTableLocation))
    append(result, value: UInt16(1)) // Number of base script records
    let latinScriptTag = FourCharacterTag(string: "latn")
    append(result, value: latinScriptTag.a)
    append(result, value: latinScriptTag.b)
    append(result, value: latinScriptTag.c)
    append(result, value: latinScriptTag.d)
    append(result, value: UInt16(result.length + 2 - baseScriptListLocation))

    append(result, value: UInt16(6)) // Offset to BaseValues table
    append(result, value: UInt16(0)) // Offset to MinMax table
    append(result, value: UInt16(0)) // Number of BaseLangSys records

    append(result, value: UInt16(0)) // Default baseline index into BaseTagList
    append(result, value: UInt16(1)) // Number of BaseCoord tables
    append(result, value: UInt16(6)) // Offset to BaseCoord table

    append(result, value: UInt16(1)) // Format
    append(result, value: Int16(200)) // Offset of baseline
...
do {
    try result.write(toFile: "/Users/litherum/tmp/outputBASE.ttf", options: .atomic)
} catch {
    fatalError()
}
